...
`afl-fuzz -Jdm`. It changes how the queue prices long inputs, which is tuned for
the forgetful case by default, it adds a hit-count high-water channel, and it
checks the harness and measures execution cost once at startup. It is off
