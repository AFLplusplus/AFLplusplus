...
/* State fuzzing mode (-J). Internal constants of the mode are in afl-fuzz.h;
   only the knobs worth changing are here. */
...
/* Percent of the queue a save channel is allowed to have created on its own.
   Past this the channel is resolving finer than it can pay for. */
#define STATE_ADMIT_PCT 25U

/* Hit count a slot must reach, and the growth over its own high-water mark it
   must show, before the high-water channel (-Jm) credits it */
...
/* Percent of channel-only entries that must have gone on to mother a coverage
   find for the channel to be considered worth its cost. Above this the bound
   leaves the channel alone however much of the queue it owns: a channel that
   is producing finds is not too fine, it is expensive and working. */
#define STATE_YIELD_PCT 10U

/* Channel-only entries needed before that share means anything */
