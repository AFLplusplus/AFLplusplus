...
struct queue_entry {
  ...
  /* State fuzzing mode (-J). */
  u32 op_count,                         /* operations performed, 0=unknown  */
...
typedef struct afl_env_vars {
  ...
  s32 afl_pizza_mode, afl_ijon_history_limit, afl_watchdog_ms,
      afl_state_admit_pct, afl_state_yield_pct, afl_ijon_admit_pct;
...
#define STATE_MODE_DEEP 0x0001U     /* d - deep-input shelf                 */
#define STATE_MODE_CONTRACT 0x0002U /* c - harness self-check at startup    */
...
#define STATE_SHELF_CELLS (STATE_SHELF_DEPTH_BUCKETS * STATE_SHELF_COST_BUCKETS)
#define STATE_SHELF_WITNESSES 4U /* competing only in its own cell, this    */
                                 /* many winners per cell                   */
  ...
  double              *shelf_avg_exec_us, *shelf_avg_len;
  u32                 *shelf_count;
...
struct custom_mutator {
  ...
  /**
   * Report how much work an input does, for a harness that knows more about
   * its own input than a byte string can express.
   *
   * Called once per queue entry, at calibration time, with the entry's bytes.
   * The mutator reports how many operations the input performs; the deep-input
   * shelf (-Jd) uses that as its achievement axis, where it would otherwise
   * fall back to the input length.
   *
   * state_id is accepted for source compatibility and ignored - nothing
   * consumes a state id any more.
   *
   * (Optional)
   *
   * @param[in] data pointer returned in afl_custom_init by this custom mutator
...
u32  count_cov_non_255_bytes(afl_state_t *, u8 *);
