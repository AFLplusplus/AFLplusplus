...
PreservedAnalyses IJONInstrumentation::run(Module &M, ModuleAnalysisManager &MAM) {
  ...
  FunctionType *hashstackFT = FunctionType::get(
    Type::getInt32Ty(Context),
    {}, // no arguments
    false // not variadic
  );
  (void)Function::Create(hashstackFT, Function::ExternalLinkage, "ijon_hashstack", &M);

  // ijon_hashint function: uint32_t ijon_hashint(uint32_t old, uint32_t val)
  FunctionType *hashintFT = FunctionType::get(
    Type::getInt32Ty(Context),
    {Type::getInt32Ty(Context), Type::getInt32Ty(Context)}, // uint32_t, uint32_t
    false // not variadic
  );
  (void)Function::Create(hashintFT, Function::ExternalLinkage, "ijon_hashint", &M);
  ...
  FunctionType *ijonXorStateFT = FunctionType::get(
    Type::getVoidTy(Context),
    {Type::getInt32Ty(Context)}, // val
    false // not variadic
  );
  (void)Function::Create(ijonXorStateFT, Function::ExternalLinkage, "ijon_xor_state", &M);
  ...
  if (!inst_blocks) {
    ...
    int total_calls = ijon_max_calls + ijon_set_calls + ijon_inc_calls + ijon_state_calls;
    if (total_calls > 0) {
...
void IJONInstrumentation::createIJONMaxVariadicFunction(Module &M) {
  ...
  // Create the ijon_max_variadic function: void ijon_max_variadic(uint32_t addr, ...)
  ...
  (void)Function::Create(ijonMaxVariadicFT, Function::ExternalLinkage, "ijon_max_variadic", &M);
}
...
int IJONInstrumentation::instrumentFunction(Function &F) {
  ...
  Function *ijonMaxVariadicFunc = F.getParent()->getFunction("ijon_max_variadic");
    ...
    for (auto &I : BB) {
      ...
      if (CallInst *call = dyn_cast<CallInst>(&I)) {
        ...
        if (Function *calledFunc = dyn_cast<Function>(calledValue)) {
          if (calledFunc->getName() == "ijon_max" || calledFunc->getName() == "ijon_max_variadic" ||
              calledFunc->getName() == "ijon_set" || calledFunc->getName() == "ijon_inc" ||
          ...
          call->print(rso);
          if (callStr.find("ijon_max") != std::string::npos || callStr.find("ijon_set") != std::string::npos ||
              callStr.find("ijon_inc") != std::string::npos || callStr.find("ijon_xor_state") != std::string::npos) {
            unsigned argCount = call->arg_size();
            if (argCount >= 1) {
              callsToTransform.push_back(call);
            }
...
uint32_t IJONInstrumentation::generateLocationHash(CallInst *call, uint32_t call_index) {
  // Use a deterministic hash based on source file path, line number, column, and call index
...
void IJONInstrumentation::transformIJONMaxCall(CallInst *call, uint32_t call_index) {
  IRBuilder<> IRB(call);
  ...
  // Get the ijon_max_variadic function
  Function *ijonMaxVariadicFunc = call->getParent()->getParent()->getParent()->getFunction("ijon_max_variadic");
  ...
  // First argument: preserve the original location hash from the macro (if available)
  // For IJON_MAX macro calls, the first argument should be the macro's location hash
  if (argCount > 0) {
    Value *firstArg = call->getArgOperand(0);
    if (firstArg->getType()->isIntegerTy()) {
      // This looks like it came from IJON_MAX macro - preserve the location hash
      args.push_back(IRB.CreateIntCast(firstArg, Type::getInt32Ty(call->getContext()), false));

      // Add the remaining arguments (skip the first one as it's the location hash)
      for (unsigned i = 1; i < argCount; i++) {
        Value *arg = castToInt64(call->getArgOperand(i), IRB);
        args.push_back(arg);
      }
    } else {
      // Direct call or non-integer first arg - generate location hash and include all args
      uint32_t locationHash = generateLocationHash(call, call_index);
      args.push_back(ConstantInt::get(Type::getInt32Ty(call->getContext()), locationHash));
...
void IJONInstrumentation::transformIJONSetCall(CallInst *call, uint32_t call_index) {
  IRBuilder<> IRB(call);

  // Get the ijon_set function
  Function *ijonSetFunc = call->getParent()->getParent()->getParent()->getFunction("ijon_set");
  ...
  Value *originalLocationHash = call->getArgOperand(0); // First argument (macro's hash)
  if (originalLocationHash->getType()->isIntegerTy()) {
    args.push_back(IRB.CreateIntCast(originalLocationHash, Type::getInt32Ty(call->getContext()), false));
  } else {
  ...
  if (originalArg->getType()->isIntegerTy()) {
    castArg = IRB.CreateIntCast(originalArg, Type::getInt32Ty(call->getContext()), false);
  } else if (originalArg->getType()->isPointerTy()) {
    castArg = IRB.CreatePtrToInt(originalArg, Type::getInt32Ty(call->getContext()));
  } else {
...
void IJONInstrumentation::transformIJONIncCall(CallInst *call, uint32_t call_index) {
  IRBuilder<> IRB(call);

  // Get the ijon_inc function
  Function *ijonIncFunc = call->getParent()->getParent()->getParent()->getFunction("ijon_inc");
