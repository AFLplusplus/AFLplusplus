...
static uint64_t s_udiv(uint64_t a, uint64_t b) { return a / b; }
__attribute__((noinline, optnone))
static uint64_t s_urem(uint64_t a, uint64_t b) { return a % b; }
__attribute__((noinline, optnone))
static int64_t s_sdiv(int64_t a, int64_t b)    { return a / b; }
__attribute__((noinline, optnone))
static int64_t s_srem(int64_t a, int64_t b)    { return a % b; }
...
int main(void) {
  ...
  fprintf(stderr, "BUG_SCALAR_DIV: r=%lx rr=%lx\n",
          (unsigned long)r, (unsigned long)rr);
  return 0;
}
