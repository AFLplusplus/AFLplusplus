...
fsrv_run_result_t __attribute__((hot)) fuzz_run_target(afl_state_t      *afl,
  ...
  if (unlikely(fsrv->use_ijon && afl->ijon_state && afl->ijon_bits)) {
    ...
    if (input_data) {
      ...
      // Use saved IJON offset directly during fastresume for consistent offset
      // calculation
      ...
      if (has_saved_ijon_state()) {
        ...
        if (saved_state && saved_state->is_initialized) {
          ...
          shared_access = (dynamic_shared_access_t *)ck_alloc(
              sizeof(dynamic_shared_access_t));
          shared_access->ijon_max_area =
              (u64 *)(fsrv->trace_bits + saved_state->ijon_offset);

        } else {

          shared_access = setup_dynamic_shared_access(
