...
int rel_on_remove(fs_relation_t *rel, u64 idx, u64 size) {
  ...
  // Compute overlap.
  u64 overlap_min = MIN(MAX(idx, rel->anchor), rel->insert);
  u64 overlap_max = MAX(MIN(idx + size, rel->anchor), rel->insert);
  u64 overlap = overlap_max - overlap_min;
...
void frameshift_stage(afl_state_t *afl) {
  ...
  u32 loss_buffer[index_count];
  memset(loss_buffer, 0, sizeof(loss_buffer));
  u32 loss_count = 0;
