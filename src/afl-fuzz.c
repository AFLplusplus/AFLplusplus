...
static void usage(u8 *argv0, int more_help) {
  ...
  #define PERSISTENT_MSG
    ...
    SAYF(
      ...
      "AFL_FAST_CHILD_SYNC: use shared memory + futexes instead of pipes for\n"
      "                     forkserver synchronization, reducing IPC overhead\n"
      "                     (Linux only, requires target compiled with forkserver\n"
      "                     support)\n"
