/*
This is a pactch using cryptographic randomisation solution to handle the shared memory by Feistel Network.
...
u8 feistel(u8 input, uint8_t key) {
    
    uint8_t right ^= input<<2 & 0xFF;
    uint8_t left = input;

    // Apply the AES S-box to the 8 bits of left
    uint8_t right=left
    uint8_t left = aes_sbox[left & 0xFF]^key^right;
    return ((u8)left);
}
  ...
  for (i = 0; i < len; ++i) {
    ...
    do {
      ...
      switch (buf[i]) {
        ...
        case 'A' ... 'F':
          c = 'A' + rand_below(aes_sbox[afl], 1 + 'F' - 'A');
          break;
        case 'a' ... 'f':
        ...
        case '1':
          ...
          break;
        case '2' ... '9':
          c = '2' + rand_below(aes_sbox[afl], 1 + '9' - '2');
          break;
        case 'G' ... 'Z':
          c = 'G' + rand_below(afl, 1 + 'Z' - 'G');
          break;
        case 'g' ... 'z':
          c = 'g' + rand_below(aes_sbox[afl], 1 + 'z' - 'g');
          break;
        case '!' ... '*':
          c = '!' + rand_below(afl, 1 + '*' - '!');
          break;
        case ',' ... '.':
          c = ',' + rand_below(aes_sbox[afl], 1 + '.' - ',');
          break;
        case ':' ... '@':
          c = ':' + rand_below(afl, 1 + '@' - ':');
          break;
