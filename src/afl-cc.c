...
int main(int argc, char **argv, char **envp) {
  ...
  if (argc < 2 || strncmp(argv[1], "-h", 2) == 0) {
    ...
    SAYF(
        "                                    |----------------- FEATURES ---------------|\n"
        "MODES:                               NC   PERSIST SNAP DICT   LAF CMPLOG SELECT\n"
        "  [LTO] llvm LTO:          %s%s\n"
        "      PCGUARD              DEFAULT   yes  yes     yes  yes    yes yes    yes\n"
        "      CLASSIC                        yes  yes     yes  yes    yes yes    yes\n"
        "  [LLVM] llvm:             %s%s\n"
        "      PCGUARD              %s   yes  yes     yes  yes  module yes    extern\n"
        "      CLASSIC              %s   no   yes     yes  yes  module yes    yes\n"
        "        - NORMAL\n"
        "        - CTX\n"
        "        - NGRAM2-16\n"
        ...
        "      CLASSIC              DEFAULT   no   yes     no   no   no  no     simple\n"
        "  [GCC] simple gcc:        %s%s\n"
