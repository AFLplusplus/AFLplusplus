...
static u8 trig_parse_mode(char *v) {

  if (!v) { return 0; }
  if (!strcmp(v, "time")) { return 1; }
  if (!strcmp(v, "off")) { return 2; }
  if (!strcmp(v, "hybrid")) { return 3; }
  return 0;

}

void setup_exec_triggers(afl_state_t *afl) {

  u8 base = trig_parse_mode(getenv("AFL_TRIG_ALL"));
  for (u32 i = 1; i <= 5; ++i) { afl->trig_mode[i] = base; }

  if (getenv("AFL_TRIG_M1")) {

    afl->trig_mode[1] = trig_parse_mode(getenv("AFL_TRIG_M1"));

  }

  if (getenv("AFL_TRIG_M2")) {

    afl->trig_mode[2] = trig_parse_mode(getenv("AFL_TRIG_M2"));

  }

  if (getenv("AFL_TRIG_M3")) {

    afl->trig_mode[3] = trig_parse_mode(getenv("AFL_TRIG_M3"));

  }

  if (getenv("AFL_TRIG_M4")) {

    afl->trig_mode[4] = trig_parse_mode(getenv("AFL_TRIG_M4"));

  }

  if (getenv("AFL_TRIG_M5")) {

    afl->trig_mode[5] = trig_parse_mode(getenv("AFL_TRIG_M5"));

  }

  char *lp = getenv("AFL_TRIG_LOG");
  if (lp) {

    afl->trig_log = fopen(lp, "w");
    if (afl->trig_log) {

      fprintf(afl->trig_log,
              "event,wall_ms,total_execs,edges,queued_items,pending_favored,"
              "fuzz_mode,starved,cur_seed,mech_mode\n");
...
void afl_trig_log(afl_state_t *afl, const char *event, u8 mode) {
  ...
  if (likely(!afl->trig_log)) { return; }

  u32 edges = count_non_255_bytes(afl, afl->virgin_bits);
  fprintf(afl->trig_log, "%s,%llu,%llu,%u,%u,%u,%u,%u,%u,%u\n", event,
          (unsigned long long)(get_cur_time() - afl->start_time),
          (unsigned long long)afl->fsrv.total_execs, edges, afl->queued_items,
          afl->pending_favored, afl->fuzz_mode, afl->starved,
          afl->current_entry, mode);
  fflush(afl->trig_log);
...
void write_stats_file(afl_state_t *afl, u32 t_bytes, double bitmap_cvg,
  ...
  fprintf(
      ...
      afl->cmplog_tightness, afl->cmplog_tightness_new, afl->cmplog_size_derive,
      afl->sync_time_us / 1000000, afl->trim_time_us / 1000000,
      afl->table_time_us / 1000000, afl->fsrv.total_execs,
...
inline void update_table_time(afl_state_t *afl, u64 *time) {

  u64 cur = get_cur_time_us();
  afl->table_time_us += cur - *time;
  *time = cur;
